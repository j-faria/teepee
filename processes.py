...
try:
    from george.basic import BasicSolver
    solver = BasicSolver
...
# epsilon to add on the diagonal of the matrices in the absence
# of observational uncertainties for computational stability.
TINY = 1.25e-12


class GaussianProcess(object):
    """
    A basic Gaussian Process object, borrowed from `george` 
    ...
    def predict_slow_but_more_readable(self, y, t, mean_only=False):
        """
        Compute the conditional predictive distribution of the model.
        
        p(yn,t | y,x) = N(mut + Syny Sy-1 (y-muy) ; Sy - Syny Sy-1 SynyT)
        p(yn,t | y,x) = N(mut + Stx Sx-1 (y-muy) ; St - Stx Sy-1 StxT)
        ...
        * **cov** ``(ntest, ntest)`` is the predictive covariance.
        ...
        B = self.kernel.value(t, self._x)
        C = self.kernel.value(self._x, self._x)
        A = self.kernel.value(t, t)
    ...
    def sample(self, t=None, size=1):
        ...
        return multivariate_gaussian_samples(cov, size, mean=self.mean(t))
    ...
    def optimize(self, x, y, yerr=TINY, sort=True, dims=None, verbose=True, **kwargs):
        """
...
class TProcess(object):
    """ A T-process object, most boilerplate also borrowed form `george` """

    def __init__(self, kernel, df, mean=None, solver=BasicSolver, **kwargs):
        self.kernel = kernel
        self._computed = False
        self._alpha = None
        self._y = None
    ...
    def predict(self, y, t, mean_only=False):
        ...
        * **cov** ``(ntest, ntest)`` is the predictive covariance.
        ...
        # the conditional predictive has different df
        newdf = self.df + y.size
        ...
        # Compute the predictive covariance.
        beta1 = np.dot(self._y, self._alpha)
        f = (self.df + beta1 - 2) / (self.df + self._x.size - 2)
        KxsT = np.ascontiguousarray(Kxs.T, dtype=np.float64)
        cov = self.kernel.value(xs)
        cov -= np.dot(Kxs, self.solver.apply_inverse(KxsT, in_place=True))
        cov *= f
    ...
    def sample_conditional(self, y, t, size=1):
        ...
        """
        newdf, mu, cov = self.predict(y, t)
        return multivariate_t_samples(cov, newdf, size, mean=mu)
    ...
    def sample(self, t=None, size=1):
        ...
        x, _ = self.parse_samples(t, False)
        cov = self.get_matrix(x)
        return multivariate_t_samples(cov, self.df, size, mean=self.mean(t))
...
def multivariate_t_samples(matrix, df, N, mean=None):
    """
    ...
    :returns samples: ``(k,)`` or ``(N, k)``
        Samples from the given multivariate t.
    """
    if mean is None:
        mean = np.zeros(N,matrix.shape[0])
    d = len(matrix)
    if df == np.inf:
        x = 1.
    else:
    ...
    z = np.random.multivariate_normal(np.zeros(d), matrix, N)
    return mean + z/np.sqrt(x)[:,None]   # same output format as random.multivariate_normal
